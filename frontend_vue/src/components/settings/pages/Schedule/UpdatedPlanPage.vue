<template>
  <section v-if="uiStore.scheduleView === 'todo_groups'" class="space-y-4">
    ...
    <div v-if="showEditor" class="rounded-lg border border-cyan-300/25 bg-slate-900/25 p-4 space-y-3">
      <input
        v-model="form.title"
      ...
      <div class="flex justify-end gap-2">
        ...
        <button class="action-btn bg-cyan-500/35 hover:bg-cyan-500/50" :disabled="!canSave" @click="savePlan">
          <Save :size="16" />
          保存计划
        </button>
      </div>
    ...
    <article v-else class="rounded-lg border border-cyan-300/20 bg-slate-900/25 p-4 text-white">
      ...
      <ol class="mt-4 space-y-2">
        ...
        >
          <span :class="['mt-1 h-2.5 w-2.5 rounded-full shrink-0', statusDotClass(item.status)]"></span>
          <div class="min-w-0">
            <p :class="['text-sm leading-relaxed', item.status === 'completed' ? 'line-through text-white/55' : 'text-white']">
              {{ item.step }}
            </p>
...
const handleCreate = () => {
  ...
  form.lines = (plan.value?.items || [])
    .map((item) => `${statusLabel(item.status)}：${item.step}${item.note ? ` ｜ ${item.note}` : ''}`)
    .join('\n')
