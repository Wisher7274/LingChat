<template>
  <section v-if="uiStore.scheduleView === 'sandbox'" class="space-y-4">
    ...
    <div v-if="error" class="rounded-lg border border-rose-400/40 bg-rose-500/15 p-3 text-sm text-rose-100">
      {{ error }}
    </div>
    ...
    <div v-if="editorVisible" class="rounded-lg border border-cyan-300/20 bg-slate-900/25 p-4 space-y-3">
      <div class="flex items-center justify-between gap-3">
        <input
    ...
    <div class="rounded-lg border border-cyan-300/20 bg-slate-900/25 p-4 space-y-3">
      ...
      <div class="flex gap-2">
        ...
        />
        <button class="action-btn bg-emerald-500/30 hover:bg-emerald-500/45" :disabled="!packageName || executing" @click="installPackage">
          <PackagePlus :size="16" />
          安装库
        </button>
      </div>
      <div class="flex gap-2">
        ...
        />
        <button class="action-btn bg-cyan-500/30 hover:bg-cyan-500/45" :disabled="!command || executing" @click="executeCommand">
          <Play :size="16" />
          执行
        </button>
      </div>
...
const saveFile = async () => {
  error.value = ''
  try {
    await http.post('/v1/chat/sandbox/write', {
...
const executeCommand = async () => {
  ...
  commandOutput.value = ''
  try {
    const result = await http.post<ExecuteResult>('/v1/chat/sandbox/execute', {
...
const installPackage = async () => {
  ...
  commandOutput.value = ''
  try {
    const result = await http.post<ExecuteResult>('/v1/chat/sandbox/install', {
      package: name,
    }, { silent: true })
    commandOutput.value = [
      `$ ${result.command || command.value}`,
