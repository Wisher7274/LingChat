<template>
  <section v-if="uiStore.scheduleView === 'tool_calls'" class="space-y-4">
    <div class="flex items-center justify-between">
      ...
      <button
        ...
        @click="uiStore.clearToolCallLogs()"
      >
        <Trash2 :size="16" />
        清空
      </button>
    ...
    <div
      v-if="uiStore.toolCallLogs.length === 0"
    ...
    <div v-else class="space-y-3">
      <article
        v-for="entry in uiStore.toolCallLogs"
      ...
      >
        <div class="flex items-center justify-between gap-3">
          <div class="flex items-center gap-2 min-w-0">
            <CheckCircle2 v-if="entry.status === 'success'" :size="18" class="text-emerald-300 shrink-0" />
            <LoaderCircle v-else-if="entry.status === 'running'" :size="18" class="text-cyan-300 animate-spin shrink-0" />
        ...
        <div class="mt-3 space-y-2">
          <div class="rounded-lg bg-black/20 p-3">
            ...
            <pre class="text-xs whitespace-pre-wrap break-words">{{ formatJson(entry.arguments || {}) }}</pre>
          </div>
