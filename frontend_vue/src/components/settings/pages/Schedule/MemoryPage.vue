<template>
  <section v-if="uiStore.scheduleView === 'memories'" class="space-y-4">
    <div v-if="showEditor" class="rounded-lg border border-cyan-300/25 bg-slate-900/25 p-4 space-y-3">
      ...
      <div class="flex justify-end gap-2">
        ...
        <button class="action-btn bg-cyan-500/35 hover:bg-cyan-500/50" :disabled="!form.content.trim()" @click="saveMemory">
          <Save :size="16" />
          保存记忆
        </button>
      </div>
    ...
    <div v-else class="space-y-3">
      ...
      >
        ...
        <div class="mt-3 flex flex-wrap items-center gap-2 text-xs text-white/55">
          <span v-if="memory.source" class="rounded bg-white/10 px-2 py-1">{{ memory.source }}</span>
...
const persist = async () => {
  await saveSchedules({ memoryNotes: memories.value })
}
