  ...
  <div class="settings-text-container">
    <MenuPage>
      ...
      <div v-for="setting in envSettings" :key="setting.key" class="">
        ...
        <Toggle :checked="setting.value.toLowerCase() === 'true'" @change="handleMemorySettingChange($event, setting)">
          开启后记忆将会自动压缩
        </Toggle>
      </div>
      <template #header>
      ...
      <div class="flex gap-3">
        ...
        <Button v-if="isFreeDialogMode" type="big" variant="danger" @click="handleClearHistory">清除历史对话</Button>
      </div>
      </MenuItem>
    </MenuPage>
  </div>
