...
function sendOrContinue() {
  console.log('[sendOrContinue] currentStatus:', gameStore.currentStatus, 'hasText:', !!inputMessage.value.trim(), 'hasDialogue:', !!uiStore.showCharacterLine)
...
function continueDialog(isPlayerTrigger: boolean): boolean {
  const wasWaitingForUser = eventQueue.getState().isWaitingForUser
  ...
  if (!wasWaitingForUser && uiStore.showCharacterLine && uiStore.showCharacterLine !== '') {
    uiStore.showCharacterLine = ''
    currentDisplayedText.value = ''
    inputMessage.value = ''
    gameStore.currentStatus = 'input'
