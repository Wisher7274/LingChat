...
def _resolve_sandbox_path(relative_path: str) -> Path:
    ...
    try:
        ...
        raise PermissionError(f"Path '{relative_path}' is outside the sandbox")
    return full_path
...
def sandbox_write_file(path: str, content: str, append: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        verified = written_content == content if not append else written_content.endswith(content)
...
def sandbox_execute_command(command: str, timeout: int = 30) -> dict[str, Any]:
    ...
    if not is_enabled:
        return {"ok": False, "error": "Sandbox command execution is disabled by administrator"}
    ...
    try:
        ...
        return {
            "ok": result.returncode == 0,
            "command": command,
            "resolved_command": command_to_run if isinstance(command_to_run, str) else " ".join(command_to_run),
...
def sandbox_install_package(package: str, timeout: int = 300) -> dict[str, Any]:
    ...
    if not PACKAGE_NAME_RE.fullmatch(package):
        return {"ok": False, "error": "Package name may only contain letters, numbers, dot, underscore and hyphen"}
    ...
    try:
        ...
        return {"ok": False, "package": package, "error": f"Package install timed out after {timeout_val} seconds"}
    except Exception as e:
        return {"ok": False, "package": package, "error": f"Package install failed: {e}"}
